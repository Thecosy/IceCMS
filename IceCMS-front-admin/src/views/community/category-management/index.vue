...
import {
  ...
  topSquareClass
} from '@/api/com_function/com_category';
...
const fetchCategories = async (pageNum = 1, limit = pageSize.value) => {
  try {
    const response = await getallSquareClass(currentPage.value, pageSize.value) as unknown as { code: number, data: CategoryResponse };
...
const handleTopChange = (category) => {
  ...
  // 实现置顶分类的逻辑
  topSquareClass(category.id).then(() => {
    ElNotification({
      title: '成功',
      message: '分类置顶状态已更新',
      type: 'success',
