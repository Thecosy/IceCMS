<template>
  <div>
    <el-dialog v-model="dialogVisible" title="添加新资源" width="500px" :before-close="handleClose">
        ...
        <el-table-column prop="author" label="作者">
          <template #default="scope">
            <div class="author-container">
              ...
              <el-avatar :src="scope.row.authorThumb" size="small"></el-avatar>
            </div>
          </template>
        </el-table-column>
...
import { deleteArticlesBatch, getAllArticles, createArticle, updateArticles, deleteArticle } from '@/api/res_function/resourse'; // 请确保路径正确
...
const handleRowClick = (row, column, event) => {
  ...
  const newPath = '/resources-management/edit-article/' + row.id
...
const confirmDeleteArticle = async (articleId) => {
  // 弹出确认框
  ElMessageBox.confirm('你确定要删除此资源吗?')
    .then(async () => {
      // 如果用户点击了确认按钮
      try {
...
const editArticleForm = ref({
  ...
  className: '',
  thumb: '',
  sortClass: '' as string | number // 修改类型为string|number
});
...
const confirmDeleteSelected = async () => {
  ...
  try {
    await ElMessageBox.confirm('你确定要删除选中的资源吗?');
    ...
    if (response && response.code === 200) {
      ...
      ElNotification({
        title: '删除成功',
        message: `成功删除${idsToDelete.length}个资源`,
        type: 'success',
      });
    } else {
