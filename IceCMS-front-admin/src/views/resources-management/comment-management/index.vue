  ...
  <div>
    <el-dialog v-model="dialogVisible" title="添加新评论" width="500px" :before-close="handleClose">
      <el-form :model="commentForm">
        ...
        <el-form-item label="资源">
          <el-select v-model="commentForm.resourceId" placeholder="请选择分类">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <el-table :data="comments" style="width: 100%">
        ...
        <el-table-column prop="resourceId" label="文章ID"></el-table-column>
        <el-table-column prop="resourceName" label="资源名">
          <template #default="scope">
            <a class="article-link" @click.prevent="navigateToArticle(scope.row.resourceId)">{{ scope.row.resourceName
            }}</a>
          </template>
        </el-table-column>
...
import { getallResourceComments, addArticleComment, updateArticlesComment, deleteArticleCommentById, getAllArticleName } from '@/api/res_function/res_comment'; // 请确保路径正确
...
const fetchArticles = async (pageNum = 1, limit = pageSize.value) => {
  try {
    ...
    const response = await getallResourceComments(pageNum, limit) as unknown as { code: number, data: CommentResponse };
...
const showAddCommentDialog = () => {
  commentForm.value = { content: '', resourceId: '', userId: '' }; // Reset form
