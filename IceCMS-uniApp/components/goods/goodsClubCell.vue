<template>
	<view class="n-position-relative" style="padding-bottom: 30rpx;" bubble="true" @click="toDetail">
		...
		<view class="n-position-absolute n-flex-row n-align-center n-justify-center" :style="{height: '50rpx', left: '0', width: '350rpx', top: (height-50)+'rpx'}">
			<n-icon name="clock" type="inverse" size="ss"></n-icon>
			<text class="n-color-inverse n-size-ss" style="margin-left: 6rpx;">26:24:32后结束</text>
		</view>
		...
		<view class="n-flex-row n-wrap-nowrap n-justify-between n-align-center" style="margin-left: 12rpx;margin-top: 10rpx;">
			<view class="n-flex-row n-wrap-nowrap n-align-center">
				<view v-for="(tag,idx) in tags" :key="idx" :class="['gc-tag', 'n-flex-row', 'n-align-center', idx%2===1?'gc-tag-yellow':'gc-tag-red']">
					<text :class="['n-color-'+(idx%2===1?'warning':'error'), 'n-size-ss']">{{tag}}</text>
				</view>
			</view>
			<n-avatars :items="members" label="avatar" :reverse="true" size="32rpx" space="-12rpx" boxStyle="margin-right:24rpx;"></n-avatars>
		</view>
