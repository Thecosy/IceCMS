<template>
	<view>
		<block v-for="(item, index) in list" :key="index">
			<view @click="jump(item)">
				<view class="post-item">
					<view class="post-item-top-user">
						<view @click.stop="toUcenter(item.userid)">
							<u-avatar class="avatar" :src="item.authorImg" 
		 ...
		 -->			
							 <text  class="username">{{ item.author.substring(0, 12) }}</text>
							 </view>
							<view>
								<text v-text="getTime(item.addTime)" class="time"></text>
							</view>
						</view>
						<view class="right">
					...
					<view class="p-footer">
						...
						<view  v-show="item.is_collection" class="p-item" @click.stop="cancelCollection(item.id, index)">
							...
							<text class="count">{{ item.loveNum }}</text>
						</view>
	...
	export default {
		...
		methods: {
			...
			addCollection(id, index) {
				...
				// }
				uni.request({
					url: API.LikeClickComments(this.list[index].id),
					success: res => {
						console.log( res)
