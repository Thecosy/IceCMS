                  ...
                  <el-main>
                    <el-row :gutter="20">
                      ...
                      <div v-for="item in squaredata" :key="item.id" class="circle-contenr-out">
                        ...
                        <div class="circle-contenr">
                          <div class="topic-header">
                            <div class="topic-header-left">
                              <router-link :to="'/otheruserinfo/' + item.userid">
                          ...
                          <div class="topic-header-right">
                            <span class="topic-date topic-circle"><a 
                                target="_blank"><b class="circle-hash"><svg width="16" height="16" viewBox="0 0 36 36">
                                    <g fill-rule="evenodd">
                                      <path
                        ...
                        <div v-if="item.image !== null && item.image != ''"  style=" padding: 30px;display: flex;flex-direction: row;width: 521px;height: 230px;margin-right: 10px;">
                          <div class="topic-img-inner" v-for="imagess in item.image" :key="imagess.id">
                            <el-image style="margin-right: 10px;max-width: 400px;height: 200px; /* 可以调整图像之间的间距 */" :src="imagess.url"
                              fit="scale-down">{{ imagess.url }}</el-image>
                          </div>
                        </div>
                                        ...
                                        <div class="topic-footer">
                                          <div class="topic-footer-left">
                                            <span class="topic-date"><b><time class="b2timeago"
                                                  datetime="2021-12-27 21:58:17" itemprop="datePublished"><span v-text="
...
export default {
  ...
  methods: {
    ...
    setUpPostReply(item) {
      this.postReplyForm.postId = item.id;
      ...
      addPlanetComment(this.postReplyForm).then((res) => {
        this.postReplyForm.content = null;
      });
    },
    setUpPostReplys(userId, id, postId) {
      this.postReplysForm.postId = postId;
      this.postReplysForm.parentId = id;
      const user = JSON.parse(window.localStorage.getItem("access-admin"));
      this.userJudje = user == null;
      if (this.userJudje) {
        // console.log("未登陆");
