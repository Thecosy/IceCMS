...
async function getArticleData() {
  try {
    ...
    // useFetchGet 直接返回数据，不需要 .data.value
    if (res) {
...
<!--                    <p class="mt-2 text-muted">暂无分类数据</p>-->
<!--                  </div>-->
<!--                </div>-->
                ...
                <div v-else class="row">
                  <div v-for="item in classlist" :key="item?.id || Math.random()" class="col-3 delay-0 list-animation-leftIn">
                    <nuxt-link :to="'/classdetal/' + (item?.id || '')">
                      <a class="cursor-pointer">
                        <div class="single-special-member">
                          <div class="wrapper">
                            <div class="img-box">
                              <el-image :src="item?.imgclass || '/default-class.png'" lazy>
                                <div slot="placeholder" class="image-slot">
                                  <img style="width:100%; height:100%; object-fit:cover;" src="../static/image/loding.gif" />
                                </div>
                                <div slot="error" class="image-slot">
                                  <div class="default-img">
                                    <i class="el-icon-picture-outline" style="font-size: 32px; color: #ddd;"></i>
                                  </div>
                                </div>
                              </el-image>
                            </div>
                            <div class="info-meta">
                              <h6 class="name">
                                {{ item?.name || '未知分类' }}
                                <span class="ml-2 text-muted fs-16"> ({{ item?.num || 0 }}) </span>
                              </h6>
                            </div>
                          </div>
