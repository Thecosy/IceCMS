...
export const getAllSquare = (query: { page: number; limit: number }) => {
  return httpRequest.get<any>(`squareComment/getAllSquare/${query.page}/${query.limit}`);
};
...
export const DelectSquareById = (id: number) => {
  return httpRequest.get<any>(`squareComment/DelectSquareById/${id}`);
};
...
export const ChangeSquareById = (id: number, content: string) => {
  return httpRequest.get<any>(`squareComment/ChangeSquareById/${id}/${content}`);
};
