...
<script>
import { newResourceClass } from '@/api/resource'
import { allResourceClass } from '@/api/resource'
import { DeleteResourceClass } from '@/api/resource'
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      let that = this
      this.loading = true
      newResourceClass(that.form).then(resp => {
        //做一个简单的返回数据判断
        console.log(resp)
        if (resp.status === 200) {
          if(resp.data == 143){
