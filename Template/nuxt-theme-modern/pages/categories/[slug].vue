  ...
  <div class="min-h-screen">
    <div class="lg:ml-64 px-6 py-8">
      <div class="max-w-6xl mx-auto">
        <!-- Category Header -->
        <div v-if="currentCategory" class="text-center mb-12">
          <div class="mb-6">
            <div 
              :style="{ backgroundColor: currentCategory.color }"
              class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4"
            >
              <Icon name="folder" class="w-8 h-8 text-white" />
            </div>
            <h1 class="text-4xl font-bold text-hao-text mb-2">{{ currentCategory.name }}</h1>
            <p class="text-xl text-hao-text-secondary mb-4">{{ currentCategory.description }}</p>
            <span class="text-hao-text-secondary">{{ currentCategory.postCount }} 篇文章</span>
          </div>
        </div>
        ...
        <div v-else class="text-center py-20">
          <Icon name="folder" class="w-16 h-16 text-hao-text-secondary mx-auto mb-4 opacity-50" />
          <h2 class="text-2xl font-bold text-hao-text mb-2">暂无文章</h2>
          <p class="text-hao-text-secondary mb-6">这个分类下还没有发布文章</p>
          <nuxt-link 
            to="/" 
...
await Promise.all([
  blogStore.fetchCategory(categorySlug.value),
  blogStore.fetchPosts(1, categorySlug.value)
])

const loadMore = async () => {
  if (blogStore.posts.hasNext && !blogStore.loading) {
    await blogStore.fetchPosts(blogStore.posts.page + 1, categorySlug.value)
  }
}

// SEO
useHead(() => ({
  title: currentCategory.value ? `${currentCategory.value.name} - 分类` : '分类',
  meta: [
    { name: 'description', content: currentCategory.value?.description || '' },
    { property: 'og:title', content: currentCategory.value ? `${currentCategory.value.name} - 分类` : '' },
    { property: 'og:description', content: currentCategory.value?.description || '' },
