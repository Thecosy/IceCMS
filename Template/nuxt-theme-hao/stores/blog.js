...
import { postApi, categoryApi, tagApi, siteApi } from '~/utils/apiService'
// 保留模拟数据作为备用
  ...
  actions: {
    ...
    async initializeData() {
      try {
        if (this.useApi) {
          // 并行获取基础数据
          const [hotPosts, tags, categories, siteSettings] = await Promise.allSettled([
            postApi.getHotPosts(),
            tagApi.getTags(),
          ...
          if (siteSettings.status === 'fulfilled') {
            // 网站设置API返回单个对象
            const settingsData = siteSettings.value
            this.siteSettings = {
              ...this.siteSettings,
              ...settingsData
            }
            console.log('网站设置已加载:', this.siteSettings)
          } else {
            console.log('网站设置加载失败，使用默认值')
          }
        } else {
          // 使用模拟数据
          this.hotPosts = mockHotPosts
    ...
    async fetchSiteSettings() {
      try {
        if (this.useApi) {
          const data = await siteApi.getSiteSettings()
          this.siteSettings = {
            ...this.siteSettings,
            ...data
          }
          console.log('网站设置已更新:', this.siteSettings)
